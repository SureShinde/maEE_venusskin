...
function addCtMenuNode(node_id, menu_id, parent_id, type, entity_id, label, format, status, status_label, position, level, target, classes, firstchild, url)
{
...
function onSaveCctMenuForm()
{
    ...
    parseNodes($$('#menu-container > .menu-node'), parentsLevel);
...
function parseNodes(nodes, parentsLevel)
{
    ...
    nodes.each(function(node) {
        ...
        //apply to all inputs
        node.getElementsBySelector('> .menu-properties > .prefix_to_change, > .menu-properties > table .prefix_to_change').each(function(input) {
            input.writeAttribute('name', input.readAttribute('name').replace('{{prefix}}', new_prefix));
        });
        
        //apply position
        var menu_position = node.getElementsBySelector('> .menu-properties > .menu-position')[0];
        menu_position.value = index;
        
        //recursive for childs
        if(node.getElementsBySelector('> .menu-properties > .children > .menu-node').length > 0)
        {
            ...
            parseNodes(node.getElementsBySelector('> .menu-properties > .children > .menu-node'), newParentsLevel);
        }
