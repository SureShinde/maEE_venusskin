...
class Aitoc_Aitquantitymanager_Model_Observer {
	...
	*/
	...
	public function onRssCatalogNotifyStockCollectionSelect($observer) {
		...
		$collection->getSelect()->columns(array('website_id' => $stockItemTable . '.website_id'));
	}

	public function recurringInventoryAdjustment(Varien_Event_Observer $observer) {
		/**
		...
		if ($order->getRecurringInventoryAdjusted()) {
			return $this;
		}

		if ($order->getStoreId() AND Mage::registry('aitoc_order_create_store_id') === null) {
			Mage::register('aitoc_order_create_store_id', $order->getStoreId());
		}
		...
		$order->setRecurringInventoryAdjusted(true);
