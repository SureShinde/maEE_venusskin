...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object {
	...
	protected function insertLogo(&$page, $store = null) {
		...
		if ($image) {
			...
			if (is_file($image)) {
				...
				$width       = $this->_pixelToPoint($image->getPixelWidth());
				$height      = $this->_pixelToPoint($image->getPixelHeight());
				...
				if ($ratio > 1 && $width > $widthLimit) {
					$width  = $widthLimit;
					$height = $width / $ratio;
				} else if ($ratio < 1 && $height > $heightLimit) {
					$height = $heightLimit;
					$width  = $height * $ratio;
				} else if ($ratio == 1 && $height > $heightLimit) {
					$height = $heightLimit;
	...
	protected function insertOrder(&$page, $obj, $putOrderId = true) {
		...
		$this->setDocHeaderCoordinates(array(25, $top, 570, $top - 70));
		...
		if ($doctor = Mage::helper('theme')->getDoctorFromOrderId($order->getId())) {
			$page->drawText(
				Mage::helper('sales')->__('Clinic Account # %s', $doctor->getClinicAccountId() ? : 'N/A'),
				35,
				($top -= 15),
				'UTF-8'
			);
	...
	protected function _pixelToPoint($pixel) {
		return $pixel * 72 / 96;
	}
}
