...
class Snowdog_ShippingPriority_Model_Matrixrate_Mysql4_Carrier_Matrixrate extends Webshopapps_Matrixrate_Model_Mysql4_Carrier_Matrixrate
{
                ...
                if (empty($exceptions)) {
                    ...
                    foreach ($data as $k => $dataLine) {
                        try {
                            ...
                            $exceptions[] = Mage::helper('shipping')->__('Duplicate Row #%s (Country "%s", Region/State "%s", City "%s", Zip From "%s", Zip To "%s", Delivery Type "%s", Value From "%s" and Value To "%s")', ($k + 1), $dataDetails[$k]['country'], $dataDetails[$k]['region'], $dataLine['dest_city'], $dataLine['dest_zip'], $dataLine['dest_zip_to'], $dataLine['delivery_type'], $dataLine['condition_from_value'], $dataLine['condition_to_value'], $dataLine['priority']);
                        }
                    }
                }
                if (!empty($exceptions)) {
