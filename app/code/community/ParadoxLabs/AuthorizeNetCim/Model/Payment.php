...
class ParadoxLabs_AuthorizeNetCim_Model_Payment extends Mage_Payment_Model_Method_Cc
	...
	protected function initializeApi($force = false) {
		...
		if (is_null($this->cim)) {
			...
			$this->cim = Mage::getModel('authnetcim/api')->init(
	...
	public function runDailyBilling() {
		if ($this->_debug) {
			Mage::log('runDailyBilling()', null, 'authnetcim.log');
		}
		...
		if (count($data) > 0) {
			foreach ($data as $pid) {
				...
				if (isset($adtl['next_cycle']) && $adtl['next_cycle'] <= time()) {
					Mage::dispatchEvent('recurring_profile_before_billed', array('profile' => $profile));
					 ...
					 */
					if (isset($adtl['outstanding']) && $adtl['outstanding'] > 0 && $profile->getBillFailedLater()) {
					...
					if ($price > 0) {
						...
						try {
							...
							if (isset($order)) {
								Mage::helper('checkout')->sendPaymentFailedEmail($order, $e->getMessage(), 'Recurring Profiles Billing');
							}
							...
							$adtl['last_error_message'] = $e->getMessage();
							$adtl['failure_count']++;
	...
	public function validate() {
		if ($this->_debug) {
			Mage::log('validate()', null, 'authnetcim.log');
		}
	...
	public function authorize(Varien_Object $payment, $amount) {
		if ($this->_debug) {
			Mage::log('authorize()', null, 'authnetcim.log');
		}
	...
	public function capture(Varien_Object $payment, $amount) {
		if ($this->_debug) {
			Mage::log('capture()', null, 'authnetcim.log');
		}
	...
	public function refund(Varien_Object $payment, $amount) {
		if ($this->_debug) {
			Mage::log('refund()', null, 'authnetcim.log');
		}
	...
	public function void(Varien_Object $payment) {
		if ($this->_debug) {
			Mage::log('void()', null, 'authnetcim.log');
		}
	...
	public function cancel(Varien_Object $payment) {
		if ($this->_debug) {
			Mage::log('cancel()', null, 'authnetcim.log');
		}
	...
	public function fetchTransactionInfo(Mage_Payment_Model_Info $payment, $transactionId) {
		if ($this->_debug) {
			Mage::log('fetchTransactionInfo(' . $transactionId . ')', null, 'authnetcim.log');
		}
	...
	public function validateRecurringProfile(Mage_Payment_Model_Recurring_Profile $profile) {
		if ($this->_debug) {
			Mage::log('validateRecurringProfile()', null, 'authnetcim.log');
		}
	}
	...
	public function submitRecurringProfile(Mage_Payment_Model_Recurring_Profile $profile, Mage_Payment_Model_Info $paymentInfo) {
		if ($this->_debug) {
			Mage::log('submitRecurringProfile()', null, 'authnetcim.log');
		}
	...
	public function getRecurringProfileDetails($referenceId, Varien_Object $result) {
		if ($this->_debug) {
			Mage::log('getRPDetails()', null, 'authnetcim.log');
		}
	}
	...
	public function canGetRecurringProfileDetails() {
		if ($this->_debug) {
			Mage::log('canGetRPDetails()', null, 'authnetcim.log');
		}
	...
	public function updateRecurringProfile(Mage_Payment_Model_Recurring_Profile $profile) {
		if ($this->_debug) {
			Mage::log('updateRP()', null, 'authnetcim.log');
		}
	...
	public function updateRecurringProfileStatus(Mage_Payment_Model_Recurring_Profile $profile) {
		if ($this->_debug) {
			Mage::log('updateRPStatus()', null, 'authnetcim.log');
		}
	...
	public function getPaymentInfo($profile_id = 0, $exclude = true) {
		if ($this->_debug) {
			Mage::log('getPaymentInfo(' . $profile_id . ')', null, 'authnetcim.log');
		}
	...
	public function getPaymentInfoById($payment_id, $raw = false, $profile_id = 0) {
		if ($this->_debug) {
			Mage::log('getPaymentInfoById(' . $payment_id . ')', null, 'authnetcim.log');
		}
	...
	public function getPaymentProfiles($profile_id = 0, $exclude = true) {
		if ($this->_debug) {
			Mage::log('getPaymentProfiles(' . $profile_id . ')', null, 'authnetcim.log');
		}
	...
	public function createCustomerPaymentProfileFromForm($input, $profile_id = 0) {
		if ($this->_debug) {
			Mage::log('createCustomerPaymentProfileFromForm()', null, 'authnetcim.log');
		}
	...
	public function deletePaymentProfile($payment_id, $profile_id = 0, $really = true) {
		if ($this->_debug) {
			Mage::log('deletePaymentProfile(' . $payment_id . ')', null, 'authnetcim.log');
		}
		...
		if (!empty($profile_id)) {
			if ($really) {
				...
				if ($this->_debug) {
					Mage::log("Deleted $payment_id.", null, 'authnetcim.log');
				}
			} else {
				...
				if ($this->_debug) {
					Mage::log("Queued $payment_id for deletion.", null, 'authnetcim.log');
				}
			}
	...
	protected function getProfileId($_customer, $payment = null) {
		if ($this->_debug) {
			Mage::log('getProfileId()', null, 'authnetcim.log');
		}
	...
	protected function createCustomerProfile($_customer, $payment = null) {
		if ($this->_debug) {
			Mage::log('createCustomerProfile()', null, 'authnetcim.log');
		}
	...
	protected function createCustomerPaymentProfile($_customer, $payment, $profile_id = 0) {
		if ($this->_debug) {
			Mage::log('createCustomerPaymentProfile()', null, 'authnetcim.log');
		}
	...
	public function updateCustomerPaymentProfile($billing_id, $data, $profile_id = 0) {
		if ($this->_debug) {
			Mage::log('updateCustomerPaymentProfile(' . $billing_id . ')', null, 'authnetcim.log');
		}
	...
	protected function createCustomerPaymentProfileRecurring($_customer, $profile, $profile_id = 0) {
		if ($this->_debug) {
			Mage::log('createCustomerPaymentProfileRecurring()', null, 'authnetcim.log');
		}
	...
	protected function bill($payment, $amount, $type = 'profileTransAuthOnly') {
		if ($this->_debug) {
			Mage::log('bill(), type=' . $type, null, 'authnetcim.log');
		}
