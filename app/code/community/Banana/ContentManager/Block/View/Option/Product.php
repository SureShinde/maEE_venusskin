...
class Banana_ContentManager_Block_View_Option_Product extends Mage_Catalog_Block_Product_Abstract
{
    protected function _prepareLayout()
    {
        ...
        $this->setTemplate('contenttype/view/option/product/'.$type.'-'.$content->getId().'.phtml');
        if(!file_exists(Mage::getBaseDir('app') . DS . 'design' . DS . $this->getTemplateFile()))
        {
            ...
            $this->setTemplate('contenttype/view/option/product/'.$type.'-'.$cct->getIdentifier().'.phtml');
            if(!file_exists(Mage::getBaseDir('app') . DS . 'design' . DS . $this->getTemplateFile()))
            {
                //applying default view.phtml
                $this->setTemplate('contenttype/view/option/product/'.$type.'.phtml');
            }
        }
    }
    
    public function getProductCollection($attributes)
    {
        $collection = Mage::getModel('catalog/product')
                        ->getCollection()
                        ->addAttributeToFilter('status', 1)
                        ->addAttributeToSelect(array_merge($attributes, array('sku', 'price', 'final_price', 'url_key')));
