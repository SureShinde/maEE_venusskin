...
class Banana_ContentManager_Block_Adminhtml_Content_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset->addField('title', 'text' , array(
            ...
            'required'  => true,
            ((($content->getUseDefaultTitle() === '1' || !$entity_id) && $contentType->getMetaTitle())?'readonly':'')  => ((($content->getUseDefaultTitle() === '1' || !$entity_id) && $contentType->getMetaTitle())?'readonly':''),
            'class'     => ((($content->getUseDefaultTitle() === '1' || !$entity_id) && $contentType->getMetaTitle())?'disabled':''),
            'name'      => 'title',
            'after_element_html'    => '<p><input '.((($content->getUseDefaultTitle() === '1' || !$entity_id) && $contentType->getMetaTitle())?'checked="checked"':'').' id="use_default_title" name="use_default_title" type="checkbox" value="1" class="checkbox" onclick="if(this.checked) { $(\'title\').value=\''.addslashes($contentType->getMetaTitle()).'\'; $(\'title\').writeAttribute(\'readonly\', \'readonly\'); $(\'title\').addClassName(\'disabled\'); }  else { $(\'title\').value=\''.addslashes($content->getData('title')).'\';  $(\'title\').removeAttribute(\'readonly\'); $(\'title\').removeClassName(\'disabled\'); }"> <label for="use_default_title" class="inherit">'.Mage::helper('contentmanager')->__('Use default title').'</label></p>'
        ));
        ...
        elseif ( Mage::registry('content_data') )
        {
            ...
            if(!$entity_id)
            {
                ...
                $form->setValues(array_merge($default_values, array(
                    'title' => $contentType->getMetaTitle(),
            ...
            else
            {
                ...
                $form->setValues(array_merge($values, array(
                    'title' => ((($content->getUseDefaultTitle() === '1' || !$entity_id) && $contentType->getMetaTitle())?$contentType->getMetaTitle():$content->getTitle()),
                    'store_id' => Mage::app()->getRequest()->getParam('store')
                )));                
            }
        }
