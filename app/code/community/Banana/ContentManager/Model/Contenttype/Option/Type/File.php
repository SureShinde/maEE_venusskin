 ...
 */
class Banana_ContentManager_Model_Contenttype_Option_Type_File extends Banana_ContentManager_Model_Contenttype_Option_Type_Default
{
    /**
     ...
     * @return Banana_ContentManager_Model_Product_Option_Type_File
     */
    ...
    protected function _validateUploadedFile()
    {
        ...
        try {
            ...
            if (isset($_SERVER['CONTENT_LENGTH']) && $_SERVER['CONTENT_LENGTH'] > $this->_getUploadMaxFilesize()) {
                ...
                Mage::throwException(
                    Mage::helper('contentmanager')->__("The file you uploaded is larger than %s Megabytes allowed by server", $value)
                );
            } else {
                switch($this->getProcessMode())
                {
                    case Banana_ContentManager_Model_ContentType_Type_Abstract::PROCESS_MODE_FULL:
                        Mage::throwException(
                            Mage::helper('contentmanager')->__('Please specify the product\'s required option(s).')
                        );
                        break;
                    default:
                        $this->setUserValue(null);
        ...
        if ($upload->isUploaded($file) && $upload->isValid($file)) {
            ...
            $this->getContentType()->getTypeInstance(true)->addFileQueue(array(
     ...
     * @return Banana_ContentManager_Model_Product_Option_Type_Default
     */
    protected function _validateFile($optionValue)
    {
        $option = $this->getOption();
        /**
         * @see Banana_ContentManager_Model_Product_Option_Type_File::_validateUploadFile()
    ...
    protected function _getValidatorErrors($errors, $fileInfo)
    {
        ...
        foreach ($errors as $errorCode) {
            if ($errorCode == Zend_Validate_File_ExcludeExtension::FALSE_EXTENSION) {
                $result[] = Mage::helper('contentmanager')->__("The file '%s' for '%s' has an invalid extension", $fileInfo['title'], $option->getTitle());
            ...
            {
                $result[] = Mage::helper('contentmanager')->__("Maximum allowed image size for '%s' is %sx%s px.", $option->getTitle(), $option->getImageSizeX(), $option->getImageSizeY());
            } elseif ($errorCode == Zend_Validate_File_FilesSize::TOO_BIG) {
                $result[] = Mage::helper('contentmanager')->__("The file '%s' you uploaded is larger than %s Megabytes allowed by server", $fileInfo['title'], $this->_bytesToMbytes($this->_getUploadMaxFilesize()));
            }
        }
        return $result;
    }
    ...
    protected function _getOptionHtml($optionValue)
    {
        ...
        try {
            if (isset($value) && isset($value['width']) && isset($value['height'])
                ...
                $sizes = $value['width'] . ' x ' . $value['height'] . ' ' . Mage::helper('contentmanager')->__('px.');
            } else {
                $sizes = '';
            }
            ...
            Mage::throwException(Mage::helper('contentmanager')->__("File options format is not valid."));
        }
    }
    ...
    protected function _createWriteableDir($path)
    {
        ...
        if (!$io->isWriteable($path) && !$io->mkdir($path, 0777, true)) {
            Mage::throwException(Mage::helper('contentmanager')->__("Cannot create writeable directory '%s'.", $path));
        }
    }
