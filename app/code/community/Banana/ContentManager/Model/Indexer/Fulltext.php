...
class Banana_ContentManager_Model_Indexer_Fulltext extends Mage_Index_Model_Indexer_Abstract
{
    /**
    ...
    protected $_matchedEntities = array(
        Banana_ContentManager_Model_Content::ENTITY => array(
    ...
    public function matchEvent(Mage_Index_Model_Event $event)
    {
        ...
        $result = true;
        if($entity != Banana_ContentManager_Model_Content::ENTITY){
            return;
        }
                ...
                if(count($attributesToSelect) > 0)
                {
                    ...
                    if($content->existsForStore($storeId))
                    {
                        foreach($attributesToSelect as $key => $attributeCode)
                        {
                            ...
                            {
                                $searchData[$attributeCode] = ltrim(str_repeat('|'.$content->getAttributeText($attributeCode), $attributesWeight[$key]), '|');
                            }
                        }
