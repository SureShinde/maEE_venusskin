...
class Banana_ContentManager_Model_Content extends Banana_ContentManager_Model_Abstract
{
    const ENTITY = 'contenttype_content';
    protected $_eventPrefix = 'banana_contenttype';
     ...
     * Delete content
     *
     * @return Banana_ContentManager_Model_Content
     */
     ...
     */
    public function getImage($attributeName, $width = NULL, $height = NULL, $original = false)
    {
        ...
        {
            return str_replace('//', '/', str_replace(':/', '://', $helper->resize($directoy, $src, $imageName, $width, $height)));
        }
    }
    ...
    public function render($element, $params = null)
    {
        ...
        else if($element instanceof Banana_ContentManager_Model_Contenttype_Option)
        {
            $option = Mage::getModel('contentmanager/contenttype_option')
                    ->getCollection()
                    ->addTitleToResult($storeId)
        ...
        else if($element instanceof Banana_ContentManager_Model_Contenttype_Layout_Field)
        {
        ...
        else if($element instanceof Banana_ContentManager_Model_Contenttype_Layout_Abstract)
        {
            $layout = $element;
    ...
    public function getContentsCollection($attributeCode, $attributes = array())
    {
        ...
        $collection->addAttributeToFilter('entity_id', array(
            'IN' => explode(',', str_replace(', ', ',', $this->getContent()->getData($this->getData($attributeCode))))
        ));
